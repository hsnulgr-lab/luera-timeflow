...
// Yönetici Girişi: Ayarlar'da tanımlı PIN ile tam erişim modunu açar.
export const ManagerLogin = ({ onBack }: { onBack: () => void }) => {
    ...
    const notSet = !settings.managerPin;

    const press = async (d: string) => {
        if (checking || notSet) return;
        setError(false);
    ...
    return (
        <div style={{ height: '100dvh', background: T.bg, color: T.ink, fontFamily: T.font, display: 'flex', flexDirection: 'column', padding: '0 22px', paddingTop: 'calc(env(safe-area-inset-top,0px) + 24px)', paddingBottom: 'calc(env(safe-area-inset-bottom,0px) + 24px)' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: 8 }}>
                ...
                <h1 style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.03em', marginTop: 12 }}>Yönetici Girişi</h1>
                <p style={{ fontSize: 13, color: error ? T.red : T.muted, marginTop: 4 }}>
                    {notSet ? 'Ayarlar → Genel\'den Yönetici PIN tanımla' : error ? 'Yanlış PIN, tekrar dene' : 'Tam erişim için PIN gir'}
                </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, maxWidth: 280, margin: '0 auto', width: '100%', opacity: notSet ? 0.4 : 1, pointerEvents: notSet ? 'none' : 'auto' }}>
