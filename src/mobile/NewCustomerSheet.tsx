...
export function NewCustomerSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    const cfDefs = fieldDefsForSector(sector, 'customer');
    ...
    const handleSave = async () => {
        ...
        setSaving(false);
        if (res) { toast.success(`${t('customer')} eklendi`); setForm({ name: '', phone: '', email: '', notes: '' }); setCfValues({}); onClose(); }
    };

    const field = { background: T.surface, border: `1px solid ${T.border}`, color: T.ink, fontFamily: T.font } as const;

    return (
        <BottomSheet open={open} onClose={onClose} title={`Yeni ${t('customer')}`}>
            <div className="space-y-4 pb-2" style={{ color: T.ink }}>
                <div>
                    ...
                    <input autoFocus value={form.name} onChange={set('name')} placeholder={`${t('customer')} adı`} className="w-full rounded-2xl px-4 py-3.5 text-[15px] outline-none" style={field} />
                </div>
                <div>
                ...
                <CustomFieldsSection defs={cfDefs} values={cfValues} onChange={setCfValues}
                    T={{ ink: T.ink, muted: T.muted, surface2: T.surface, border2: T.border, rSm: 16 }} />
                <div>
                    <label className="mb-1.5 block text-[12px] font-semibold" style={{ color: T.muted }}>Not <span style={{ opacity: 0.6 }}>(opsiyonel)</span></label>
                    <textarea value={form.notes} onChange={set('notes')} rows={2} placeholder="Kayıt hakkında not" className="w-full resize-none rounded-2xl px-4 py-3.5 text-[15px] outline-none" style={field} />
                </div>
                ...
                <button disabled={!canSave} onClick={handleSave}
                    ...
                    {saving ? 'Kaydediliyor…' : `${t('customer')} Ekle`}
                </button>
            </div>
        </BottomSheet>
    );
