...
const DAY_JS     = [1,2,3,4,5,6,0]; // JS weekday for each label index

function getSchedule(member: Staff): boolean[] {
  if (!member.workingHours) {
    // default: Mon–Sat open, Sun off
...
// ── Form types ────────────────────────────────────────────────────────────────
// Luera paleti — tasarımla uyumlu (src/utils/palette.ts ile aynı)
...
export const StaffPage = () => {
  ...
  const statCards = [
    ...
    { icon: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="12" height="12" rx="2" stroke={T.orange} strokeWidth="1.4"/><path d="M5 8h6M8 5v6" stroke={T.orange} strokeWidth="1.4" strokeLinecap="round"/></svg>, val: weekResCount, lbl: 'Bu Hafta', icoStyle: { background:'rgba(255,90,31,0.08)' }, valColor: T.orange },
  ...
  return (
    <div style={{ flex:1, minHeight:0, display:'flex', flexDirection:'column', overflowY:'auto', background: T.surface, padding:'24px 28px 40px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'18px', flexWrap:'wrap', gap:'12px' }}>
        ...
        <button onClick={openAdd} style={{ display:'flex', alignItems:'center', gap:'7px', background:T.ink, color:T.cream, border:'none', borderRadius:T.rSm, padding:'9px 16px', fontSize:'13px', fontWeight:650, cursor:'pointer', fontFamily:'inherit', transition:'background .15s' }}
      ...
      {isLoading ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'56px 24px', gap:'12px', background:T.surface, border:`1px solid ${T.border}`, borderRadius:T.r }}>
          ...
          <button onClick={openAdd} style={{ marginTop:'4px', display:'flex', alignItems:'center', gap:'6px', background:T.ink, color:T.cream, border:'none', borderRadius:T.rSm, padding:'9px 16px', fontSize:'13px', fontWeight:650, cursor:'pointer', fontFamily:'inherit' }}>
            ...
            return (
              ...
              >
                ...
                <div style={{ padding:'18px 18px 14px', display:'flex', alignItems:'flex-start', gap:'14px', borderBottom:`1px solid ${T.border}` }}>
                  <div style={{ width:46, height:46, borderRadius:'50%', background:T.ink, color:T.cream, display:'grid', placeItems:'center', fontSize:'16px', fontWeight:900, flexShrink:0, letterSpacing:'-0.02em' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'5px', fontSize:'11px', fontWeight:600, color:T.muted, marginTop:'5px' }}>
                      <span style={{ width:6, height:6, borderRadius:'50%', background:T.muted2, flexShrink:0 }}/>
                      Aktif
                    </div>
                  </div>
                ...
                <div style={{ padding:'12px 16px', display:'flex', flexDirection:'column', gap:'6px' }}>
                  ...
                  <div style={{ display:'flex', gap:'3px' }}>
                    {DAY_LABELS.map((d,di) => {
                      ...
                      return (
                        <div key={di} style={{ width:26, height:26, borderRadius:6, display:'grid', placeItems:'center', fontSize:'9px', fontWeight:800, background: isToday ? T.orange : isWork ? T.ink : T.surface2, color: isToday ? T.ink : isWork ? T.cream : T.muted2, transition:'all .15s' }}>
                          {d}
                        </div>
                      );
                    })}
      ...
      <div style={{ position:'fixed', top:0, right:0, bottom:0, width:340, background:T.surface, borderLeft:`1px solid ${T.border}`, zIndex:88, transform: panelOpen ? 'translateX(0)' : 'translateX(100%)', transition:'transform .3s cubic-bezier(.4,0,.2,1)', boxShadow: panelOpen ? T.shadowLg : 'none', display:'flex', flexDirection:'column' }}>
        ...
        {selMember && (
          <div style={{ flex:1, overflowY:'auto', padding:'22px' }}>
            {/* Avatar */}
            <div style={{ width:60, height:60, borderRadius:'50%', background:T.ink, color:T.cream, display:'grid', placeItems:'center', fontSize:'22px', fontWeight:900, margin:'0 auto 12px' }}>
            ...
            <div style={{ marginBottom:'18px' }}>
              ...
              <div style={{ display:'flex', gap:'3px', marginBottom:'8px' }}>
                {DAY_LABELS.map((d,di) => {
                  const sched  = getSchedule(selMember);
                  const isWork  = sched[di];
                  const isToday = di === todayIdx;
                  return (
                    <div key={di} style={{ width:26, height:26, borderRadius:6, display:'grid', placeItems:'center', fontSize:'9px', fontWeight:800, background: isToday ? T.orange : isWork ? T.ink : T.surface2, color: isToday ? T.ink : isWork ? T.cream : T.muted2 }}>
                      {d}
                    </div>
                  );
                })}
            ...
            <div style={{ display:'flex', gap:'8px', marginTop:'4px' }}>
              <button onClick={()=>{closePanel();openEdit(selMember);}} style={{ flex:1, padding:'10px', borderRadius:'999px', background:T.ink, color:T.cream, border:'none', fontSize:'12px', fontWeight:700, cursor:'pointer', fontFamily:'inherit', transition:'background .15s' }}
                onMouseEnter={e=>(e.currentTarget.style.background='#2a2a2a')} onMouseLeave={e=>(e.currentTarget.style.background=T.ink)}>
                Düzenle
              </button>
              <button onClick={()=>handleDelete(selMember)} style={{ padding:'10px 14px', borderRadius:'999px', background:'none', color:T.muted, border:`1px solid ${T.border2}`, fontSize:'12px', fontWeight:700, cursor:'pointer', fontFamily:'inherit', transition:'all .15s' }}
      ...
      {/* ── Add/Edit Modal ── */}
      {showModal && (
        <div style={{ position:'fixed', inset:0, zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(14,14,14,0.4)', backdropFilter:'blur(4px)' }} onClick={()=>setShowModal(false)}>
          <div style={{ background:T.surface, borderRadius:'20px', padding:'28px', width:480, maxWidth:'90vw', boxShadow:T.shadowLg, maxHeight:'90vh', overflowY:'auto', animation:'modalIn .3s cubic-bezier(.22,.8,.2,1) both' }} onClick={e=>e.stopPropagation()}>
                  ...
                  {form.workingHours.map(h=>(
                    <div key={h.day} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'5px 0', opacity:h.isOff?0.5:1 }}>
                      ...
                      <label style={{ display:'flex', alignItems:'center', gap:'5px', cursor:'pointer' }}>
                        <input type="checkbox" checked={!h.isOff} onChange={e=>updateHour(h.day,'isOff',!e.target.checked)} style={{ width:13, height:13 }}/>
                        <span style={{ fontSize:'11px', color:T.muted }}>{h.isOff?'Kapalı':'Açık'}</span>
                      </label>
                      {!h.isOff && <>
                        <input type="time" value={h.start} onChange={e=>updateHour(h.day,'start',e.target.value)} style={{ padding:'4px 8px', border:`1px solid ${T.border2}`, borderRadius:T.rXs, fontSize:'11px', fontFamily:'inherit', color:T.ink, background:T.surface2, outline:'none' }}/>
                        <span style={{ color:T.muted2, fontSize:'11px' }}>—</span>
                        <input type="time" value={h.end} onChange={e=>updateHour(h.day,'end',e.target.value)} style={{ padding:'4px 8px', border:`1px solid ${T.border2}`, borderRadius:T.rXs, fontSize:'11px', fontFamily:'inherit', color:T.ink, background:T.surface2, outline:'none' }}/>
                      </>}
                    </div>
                  ))}
                </div>
            ...
            <div style={{ display:'flex', gap:'8px', justifyContent:'flex-end', paddingTop:'18px', borderTop:`1px solid ${T.border}` }}>
              ...
              <button onClick={handleSave} disabled={saving||!form.name.trim()} style={{ padding:'9px 18px', borderRadius:T.rSm, border:'none', background:form.name.trim()?T.ink:T.surface3, color:form.name.trim()?T.cream:T.muted2, fontSize:'13px', fontWeight:650, cursor:form.name.trim()?'pointer':'not-allowed', fontFamily:'inherit', transition:'background .15s' }}>
